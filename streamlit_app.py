...
div[data-testid="stTextInput"] input, div[data-testid="stTextArea"] textarea {
    border:2px solid #BFD3C8 !important; border-radius:8px !important; background:#FFFFFF !important;
}
...
def render_goals(goals3, subj_used, bu_used, stage_used):
    ...
    st.markdown(
        ...
        "実際のお子さんの様子とは異なる場合もありますので、"
        "<b>参考程度</b>にご覧いただき、必要に応じて段階や文章を調整してください。</div>"
        ...
        for i, it in enumerate(items):
            ...
            if edit:
                ...
                if not it.get("manabi"):
                    if c3.button("段階▲", key=f"up_{pillar}_{i}",
                                 use_container_width=True):
                        ni, msg = core.goal_change_stage(data, it, 1)
                        if msg:
                            st.toast(msg)
                        else:
                            _replace(pillar, i, ni, k)
                    if c4.button("段階▼", key=f"dn_{pillar}_{i}",
                                 use_container_width=True):
                        ni, msg = core.goal_change_stage(data, it, -1)
                        if msg:
                            st.toast(msg)
